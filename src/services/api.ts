...
// Company Config
const defaultCompanyId = import.meta.env.VITE_DEFAULT_COMPANY_ID;

// --- VALIDACIÓN ESTRICTA ---
if (!supabaseUrl || !supabaseKey) {
  throw new Error("Supabase URL o KEY no están definidas en las variables de entorno");
}
...
if (!defaultCompanyId) {
  throw new Error("VITE_DEFAULT_COMPANY_ID no está definida en las variables de entorno");
}
...
class AppApi {
  ...
  async addProduct(product: Partial<Product>): Promise<boolean> {
    ...
    const { error } = await supabase
        .from('products')
        .insert([{ ...payload, is_deleted: false, company_id: defaultCompanyId }]);
  ...
  private async sendEmailWithPdf(toEmail: string, toName: string, brand: string, model: string, pdfUrl: string): Promise<boolean> {
      try {
          if (!emailJsServiceId || !emailJsTemplateId || !emailJsPublicKey) {
              console.warn("EmailJS Service/Template/Key no están definidos en variables de entorno.");
              // Opcional: throw error si es crítico
              return false;
          }
  ...
  async updateCompanyInfo(info: CompanyInfo): Promise<boolean> {
      ...
      if (data) {
          ...
          const { error } = await supabase.from('settings').insert({ ...payload, company_id: defaultCompanyId });
          if (error) throw error;
      }
      return true;
  }
