...
// Se priorizan variables de entorno VITE_ para mayor compatibilidad
const SUPABASE_URL = getEnv('VITE_SUPABASE_URL') || 'https://reqsaffzqrytnovzwicl.supabase.co'; 
const SUPABASE_ANON_KEY = getEnv('VITE_SUPABASE_ANON_KEY') || getEnv('VITE_SUPABASE_KEY') || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJlcXNhZmZ6cXJ5dG5vdnp3aWNsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjQ4NjIxMzgsImV4cCI6MjA4MDQzODEzOH0.PlAKMfoP1Ji0pNEifMIuJMgQFSQA_BOlJRUGjjPnj9M';
...
class AppApi {
  ...
  async isAuthorizedAdmin(email: string): Promise<boolean> {
  ...
  async extractProductFromPdf(file: File): Promise<Partial<Product> | null> {
    try {
        ...
        // 2. Initialize Gemini using the standard process.env.API_KEY as per coding guidelines
        ...
        // 4. Call Gemini Model - Using 'gemini-3-flash-preview' for basic text extraction task
        ...
        // 5. Parse Response - Correctly accessing .text property
